...
public class LibroRepository {
    ...
    public List<Libro> buscarPorAutor(String nombreAutor) {
        ...
        for (Libro libro : listaLibros) {
            if (libro.getAutor().toLowerCase().contains(nombreAutor.toLowerCase())) {
    ...
    public int totalLibrosPorAnio(int anio) { // <-- Este nombre debe ser exacto
        int contador = 0;
        for (Libro libro : listaLibros) {
    ...
    public List<Libro> listarOrdenados() {
        List<Libro> listaCopia = new ArrayList<>(listaLibros);
        listaCopia.sort(Comparator.comparingInt(Libro::getFechaPublicacion));
        return listaCopia;
    }
}
